...
SLURM_CPUS=`sudo -u $ADMIN_USERNAME ssh worker0 '( nproc --all )'` >> /tmp/azuredeploy.log.$$ 2>&1
echo SLURM CPU is $SLURM_CPUS >> /tmp/azuredeploy.log.$$ 2>&1
...
sudo -u $ADMIN_USERNAME ssh worker0 '( free -m )' > /tmp/noderam.slurm
...
sudo -u $ADMIN_USERNAME ssh worker0 '( lscpu|grep Thread|cut -d ":" -f 2 )' > /tmp/nodethreads.slurm
